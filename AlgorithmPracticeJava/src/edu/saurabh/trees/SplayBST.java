...
public class SplayBST<Key extends Comparable<Key>, Value>  {
    ...
    public Value get(Key key) {
        root = splay(root, key);
        // if key not found,splay operation will bring the last node on path to the root, so check key
    ...
    public void remove(Key key) {
        ...
        if (cmp == 0) {
            if (root.left == null) {
                ...
                Node x = root.right;
                root = root.left;
                splay(root, key);// bring best replacement of key to the root
                root.right = x;
            }
        }
    ...
    * **********************************************************************/
    ...
    private Node splay(Node h, Key key) {
       ...
       if(cmp1 < 0) {
    	   ...
    	   if(h.left ==null) return h; // key not in tree, so we're done
    	   
    	   int cmp2 = key.compareTo(h.left.key);
    	   
    	   if(cmp2 <0) { // zig-zig case,2  right rotations required after subtree is splayed,first at grandparent, then at parent
    		   h.left.left = splay(h.left.left,key);// first splay subtree
    		   h = rotateRight(h);
    	   }else if(cmp2 > 0) { // zig-zag case,1 left rotation then 1 right after subtree is splayed, first at parent, then at grand parent
    	   ...
    	   if(h.right ==null) return h; //// key not in tree, so we're done
    	   
    	   int cmp2 = key.compareTo(h.right.key);
    	   
    	   if(cmp2 > 0) { // // zig-zig case,2  left rotations required,first at grandparent, then at parent
    		   h.right.right = splay(h.right.right,key);
    		   h = rotateLeft(h);
    	   }else if(cmp2 <0) {// zig-zag case,1 left rotation then 1 right, first at parent, then at grand parent
